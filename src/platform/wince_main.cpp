...
static void AC_DrawFrame(HDC dc)
{
    BITMAPINFO bmi;
    ZeroMemory(&bmi, sizeof(bmi));
    bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
    bmi.bmiHeader.biWidth = g_engine.renderer.width;
    bmi.bmiHeader.biHeight = -(LONG)g_engine.renderer.height;
    ...
    StretchDIBits(
        dc,
        0, 0,
        g_engine.renderer.width,
        g_engine.renderer.height,
        0, 0,
        g_engine.renderer.width,
        g_engine.renderer.height,
        g_engine.renderer.pixels,
        &bmi,
        DIB_RGB_COLORS,
        SRCCOPY);
}
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE previous, LPTSTR commandLine, int show)
{
    ...
    if (!AC_EngineInit(&g_engine, 320, 240))
        return 1;
